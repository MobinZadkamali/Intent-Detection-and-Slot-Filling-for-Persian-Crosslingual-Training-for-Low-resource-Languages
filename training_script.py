...
# from transformers import AutoModelForTokenClassification  ## this one works for all (BERT,RoBERTA) modes with only a dense layer at end
...
if __name__ == '__main__':
    ...
    bert_model = 'HooshvareLab/bert-fa-zwnj-base' #'bert-base-uncased'
    ...
    Data["val_masks"] = [[i < Data["val_lens"][j] + 2 for i in range(len(ii))] for j, ii in enumerate(Data["val_inputs"])]
    Data["val_inputs"] = Data["val_inputs"].astype('int64')
    Data["val_tags"] = Data["val_tags"].astype('int64')

    Data["test_masks"] = [[i < Data["test_lens"][j] + 2 for i in range(len(ii))] for j, ii in enumerate(Data["test_inputs"])]

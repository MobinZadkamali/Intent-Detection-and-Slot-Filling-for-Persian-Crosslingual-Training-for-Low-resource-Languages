...
from models.transformer.seqTagger import tokenize_and_pad_text_for_train, cut_at
from transformers import AutoConfig
from models.transformer import tagger_trainer
from models.transformer import persian_tagger_trainer
...
from transformers import \
    AutoModelForTokenClassification  ## this one works for all (BERT,RoBERTA) modes with only a dense layer at end
...
def get_Data(Data):
    ...
    Data["pr_tr_inputs"] = torch.tensor(Data["pr_tr_inputs"])
    ...
    Data["pr_tr_tags"] = torch.tensor(Data["pr_tr_tags"])
    ...
    Data["pr_tr_intents"] = torch.tensor(Data["pr_tr_intents"])
    ...
    Data["pr_tr_masks"] = torch.tensor(Data["pr_tr_masks"])
...
if __name__ == '__main__':
    ...
    Data["pr_tr_masks"] = [[i < Data["pr_tr_lens"][j] + 2 for i in range(len(ii))] for j, ii in enumerate(Data["pr_tr_inputs"])]
    Data["pr_tr_inputs"] = Data["pr_tr_inputs"].astype('int64')
    Data["pr_tr_tags"] = Data["pr_tr_tags"].astype('int64')
    ...
    tagger_trainer.run_transformer_trainer(Data, batch_size=bs, FULL_FINETUNING=FULL_FINETUNING, model=model,
                                           ...
                                           lr=lr, epochs=20, save_dir=save_path)

    persian_tagger_trainer.run_transformer_trainer(Data, batch_size=bs, FULL_FINETUNING=FULL_FINETUNING, model=model,
                                           tokenizer=tokenizer,
                                           device=device, validation_func=val_fun,
                                           lr=5e-5, epochs=20, save_dir=save_path)

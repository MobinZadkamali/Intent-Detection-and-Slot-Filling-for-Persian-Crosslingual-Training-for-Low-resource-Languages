...
from .seqTagger import get_ouputs_fusion1, if_final_layer_is_crf
...
def run_transformer_trainer(Data, model_tag, model_class, model_graph, batch_size, FULL_FINETUNING, model, tokenizer,
                            device, slot_label_lst, intent_label_lst, max_len, validation_func=None,
    ...
    optimizer_grouped_parameters2 = [
        {'params': [p for n, p in named_params if not any(nd in n for nd in no_decay2)], 'weight_decay': 0.01},
        {'params': [p for n, p in named_params if any(nd in n for nd in no_decay2)], 'weight_decay': 0.0}
    ]
    ...
    optimizer2 = BertAdam(optimizer_grouped_parameters2, lr=0.01, warmup=0.1,
                         t_total=total_steps)
    ...
    model_tag.init_weights(initrange=0.2)
    model_class.init_weights(initrange=0.2)

    encoder_info_filter = lambda info: info

    class_loss_function = nn.NLLLoss(size_average=False)
    tag_loss_function = nn.NLLLoss(size_average=False)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            b_input_mask1 = b_input_mask[:, :int(max(lens))]
            active_loss = torch.reshape(b_input_mask1, (-1,)) == 1
            active_logits = tag_scores.contiguous().view(-1, len(slot_label_lst.keys()))[active_loss]
            b_labels1 = b_labels[:, :int(max(lens))]
            active_labels =torch.reshape(b_labels1, (-1,)) [active_loss]

            tag_loss = tag_loss_function(active_logits, active_labels)
            class_loss = class_loss_function(class_scores, b_intents)
            losses.append([tag_loss.item() / sum(lens).cpu(), class_loss.item() / len(lens)])
            total_loss2 = 0.5 * tag_loss + (1 - 0.5) * class_loss
        ...
        print("Average train loss: {}, {}".format(avg_train_loss, mean_loss))
        ...
        # our validation set.
        tr_true_slots, tr_pred_slots, tr_true_intents, tr_pred_intents = get_predictions(model,
                                                                                         model_tag,
                                                                                         model_class,
                                                                                         model_graph,
                                                                                         max_len,
        ...
        val_true_slots, val_pred_slots, val_true_intents, val_pred_intents = get_predictions(model,
                                                                                             model_tag,
                                                                                             model_class,
                                                                                             model_graph,
                                                                                             max_len,
        ...
        test_true_slots, test_pred_slots, test_true_intents, test_pred_intents = get_predictions(model,
                                                                                                 model_tag,
                                                                                                 model_class,
                                                                                                 model_graph,
                                                                                                 max_len,
...
def get_predictions(model, model_tag, model_class, model_graph, max_len, dataloader, device, lens, dict2, inte2, starts,
                    ):
    predictions_slots, true_slots, predictions_intents, true_intents = get_ouputs_fusion1(model, model_tag, model_class, model_graph,
                                                                                          max_len, dataloader, device)

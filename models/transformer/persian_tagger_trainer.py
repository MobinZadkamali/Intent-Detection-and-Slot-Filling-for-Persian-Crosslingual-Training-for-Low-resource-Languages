...
def run_transformer_trainer(Data, batch_size, FULL_FINETUNING, model, tokenizer,
    ...
    pr_tr_inputs = Data["pr_tr_inputs"]
    pr_tr_masks = Data["pr_tr_masks"]
    pr_tr_tags = Data["pr_tr_tags"]
    pr_tr_intents = Data["pr_tr_intents"]
    ...
    pr_tr_lens = Data["pr_tr_lens"]
    pr_tr_starts = Data["pr_tr_starts"]
    val_lens = Data["val_lens"]
    val_starts = Data["val_starts"]
    test_lens = Data["test_lens"]
    test_starts = Data["test_starts"]

    pr_tr_tokens = Data["pr_tr_tokens"]
    val_tokens = Data["val_tokens"]
    test_tokens = Data["test_tokens"]

    pr_train_data = TensorDataset(pr_tr_inputs, pr_tr_masks, pr_tr_tags, pr_tr_intents, torch.Tensor(pr_tr_lens))
    pr_train_sampler = RandomSampler(pr_train_data)
    pr_train_dataloader = DataLoader(pr_train_data, sampler=pr_train_sampler, batch_size=batch_size)
    ...
    total_steps = len(pr_train_dataloader) * epochs
    ...
    for epoch in range(epochs):
        ...
        total_loss = 0
        # Training loop
        pr_train_dataloader = tqdm(pr_train_dataloader, mininterval=60)
        ...
        avg_train_loss = total_loss / len(pr_train_dataloader)
        ...
        # our validation set.
        tr_true_slots, tr_pred_slots,  tr_true_intents, tr_pred_intents = get_predictions(model, pr_train_dataloader, device,
                                                    pr_tr_lens, model.config.dict2, model.config.inte2, pr_tr_starts)
